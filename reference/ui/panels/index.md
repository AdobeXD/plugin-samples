...
Panels appear on the left hand side of the app when user clicks on the plugins icon at the bottom left side. Panels do not prevent user interaction with the underlying document and can listen to user's selection changes. Panels are dismissed in any one of the following manners:

* The user clicks on the "back arrow" button at the top
* The user clicks on one of the other options for the left hand side of the app, such as Layers panel or Assets panel
...
As you can see in the example above, everytime user's `selection` changes, your third (optional) function, `update` will trigger. You will have access to both `selection` and `documentRoot` as parameters in the function. This means that your panel can display dynamic content based on what user has selected in the document.
...
```
Refer to the panel lifecycle documentations to learn more about these methods:
- [Show](./show.md)
- [Hide](./hide.md)
- [Update](./update.md)
